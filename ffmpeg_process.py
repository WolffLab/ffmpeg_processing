...
import os
from settings import ffmpeg_exec_path, output_path, mat_file_path
import time
import scipy.io
...
for i in range(len(files)-1):
    ...
    if (trial2[i] == trial2[i+1]):
        ...
        output_file_name = "{v}_{name}_Sess_{session}_Trial{trial_num}.mp4".format(v=view_short, name = process_file_names(files[i]).split('.')[0], session = trial1[i], trial_num = trial2[i])
        command = '{ffmpeg} -hwaccel cuda -i "{file1}" -i "{file2}" -filter_complex "[0:v]trim={beg_vid1}:{end_vid1},setpts=PTS-STARTPTS[v0];[1:v]trim={beg_vid2}:{end_vid2},setpts=PTS-STARTPTS[v1];[v0][v1]concat=n=2:v=1:a=0[out]" -map "[out]" -c:v libx264 -pix_fmt yuv420p -preset superfast -crf 23 "{out_path}\\{out_file}"'.format(ffmpeg = ffmpeg_exec_path, file1 = files[i], file2 = files[i+1], beg_vid1 = frames1[i]/90, end_vid1 = frames2[i]/90, beg_vid2 = frames1[i+1]/90, end_vid2 = frames2[i+1]/90, out_path = output_path, out_file = output_file_name)
        print(command)
        os.system(command)
    #If not then crop the video individually
        ...
        view_short = view[i][0].capitalize()
        output_file_name = "{v}_{name}_Sess_{session}_Trial{trial_num}.mp4".format(v=view_short, name = process_file_names(files[i]).split('.')[0], session = trial1[i], trial_num = trial2[i])
        command = '{ffmpeg} -hwaccel cuda -y -i "{file}" -ss {num1} -c:v libx264 -pix_fmt yuv420p -frames:v {num2} -preset superfast -crf 23 "{out_path}\\{out_file}"'.format(ffmpeg = ffmpeg_exec_path, file = files[i], num1 = frames1[i]/90, num2 = frames2[i] - frames1[i], out_path = output_path, out_file = output_file_name)
        print(command)
        os.system(command)
t1 = time.time()
total = t1-t0
